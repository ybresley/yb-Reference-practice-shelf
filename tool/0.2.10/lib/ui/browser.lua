...
local function draw_settings(ctx, state)
  ...
  if reaper.ImGui_BeginPopupModal(ctx, "Settings", nil, reaper.ImGui_WindowFlags_AlwaysAutoResize()) then
    ...
    -- The update-done popup stacks OVER this modal the moment an update lands
    -- (popups.update_done, site "settings" — always allowed to open here):
    -- during "Updating…" this modal is exactly what the user is watching, so
    -- completion must announce itself right on top of it, not wait for Close.
    local done_action = popups.update_done(ctx, state, "settings", true)
    action = action or done_action
