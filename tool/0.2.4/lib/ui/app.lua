...
-- The post-update restart popup fires ONCE per landed update (an update can't
-- run twice in one session — finishing needs a restart — so once per session
-- is once per update). Module-level like close_armed: view-only scratch.
...
function app.frame(ctx, state)
  ...
  if visible then
    ...
    -- The post-update restart popup (2026-08-05, user-requested during the
    -- update feature's live tests): the moment an update has landed, a small
    -- modal offers the one-click restart instead of leaving the instruction
    -- buried in Settings. Opened once, and only while NO other popup is up —
    -- the Settings modal may be showing the same news, and ImGui can't stack
    -- a modal from outside another's scope anyway; this then appears the
    -- moment Settings closes. Lives on the working view (always present);
    -- the restart itself is reported as an action — reaper.* is the entry
    -- script's job.
    if state.update and state.update.phase == "done" and not update_popup_shown then
      local blocked = false
      if HAS_POPUP_GUARD then
        blocked = reaper.ImGui_IsPopupOpen(ctx, "",
    ...
    if reaper.ImGui_BeginPopupModal(ctx, "yb_Reference updated###yb_update_done", nil,
      ...
      reaper.ImGui_TextColored(ctx, theme.tokens.TEXT_PRIMARY,
        string.format("Updated to v%s.", (state.update and state.update.installed) or "?"))
      if state.can_restart then
        reaper.ImGui_TextColored(ctx, theme.tokens.TEXT_SECONDARY,
          "Restart the tool to start using the new version.")
        reaper.ImGui_Dummy(ctx, 0, 4)
        if reaper.ImGui_Button(ctx, "Restart now", theme.metrics.POPUP_BTN_W + 30) then
          -- The modal blocks every other control, so the frame's single
          -- action slot is free by construction.
          action = { type = "restart_tool" }
          reaper.ImGui_CloseCurrentPopup(ctx)
        end
        reaper.ImGui_SameLine(ctx)
        -- Later = just dismiss; Settings' UPDATES row keeps carrying the
        -- "close and reopen" reminder until the restart really happens.
        if reaper.ImGui_Button(ctx, "Later", theme.metrics.POPUP_BTN_W) then
          reaper.ImGui_CloseCurrentPopup(ctx)
        end
      else
        reaper.ImGui_TextColored(ctx, theme.tokens.TEXT_SECONDARY,
        ...
        reaper.ImGui_Dummy(ctx, 0, 4)
        if reaper.ImGui_Button(ctx, "OK", theme.metrics.POPUP_BTN_W) then
          reaper.ImGui_CloseCurrentPopup(ctx)
        end
      end
      reaper.ImGui_EndPopup(ctx)
