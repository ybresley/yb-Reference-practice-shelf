...
local ud = { shown = false, owner = nil, hold = 0 }
...
function popups.update_done(ctx, state, site, can_open)
  ...
  if u and u.phase == "done" and not ud.shown and can_open then
    ...
    -- How many drawn frames the centring below is enforced for (see there).
    ud.hold = 3
  ...
  -- stacked, the working view otherwise. Enforced for the popup's FIRST FEW
  -- frames only, then released: a single appearing-frame hint was observed
  -- being ignored on the stacked modal (2026-08-05 live run — the popup
  -- landed at ImGui's remembered spot), and holding it for a few frames wins
  -- that fight; releasing afterwards keeps the popup draggable (per-frame
  -- assertion pinned it, which the user rejected).
  if ud.hold > 0 then
    local wx, wy = reaper.ImGui_GetWindowPos(ctx)
    local ww, wh = reaper.ImGui_GetWindowSize(ctx)
    reaper.ImGui_SetNextWindowPos(ctx, wx + ww * 0.5, wy + wh * 0.5, 0, 0.5, 0.5)
  end
  ...
  if reaper.ImGui_BeginPopupModal(ctx, "yb_Reference updated###yb_update_done", nil,
    ...
    -- Count down the centring hold on frames the popup actually drew.
    if ud.hold > 0 then ud.hold = ud.hold - 1 end
