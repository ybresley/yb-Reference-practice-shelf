...
-- (~485x realtime): LUFS-I and LUFS-M max ~2.1 ms per second of audio, true peak
-- ~4.3 ms. So a 5-second reference is imperceptible (~40 ms all in), while a
-- 10-minute file would freeze the window for ~5 seconds if done in one go. Doing
-- ONE pass per frame caps that at the single most expensive pass, and the sound is
-- playable throughout — its loudness simply fills in a moment later.
--
...
-- CalculateNormalization's, confirmed in the Phase 0 benchmark. Only these three
...
local MODES = {
  ...
  { key = "lufs_m_max", mode = 4 }, -- loudest momentary window
